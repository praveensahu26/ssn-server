...
/**
 * Register a mobile user. A "reporter" account is created as `reporter_pending`
 * and stays that way until an admin approves it.
 */
const register = async (body) => {
  const { name, email, mobile, password, role = 'user', documents = [] } = body;
  ...
  const user = await User.create({
    ...
    mobile,
    password,
    role: isReporter ? 'reporter_pending' : 'user',
    status: 'active',
    ...(isReporter && {
      reporterProfile: {
        documents,
...
const loginUserWithEmailOrMobile = async (identifier, password) => {
  const user = await getUserByEmailOrMobile(identifier);

  if (!user) {
    throw new ApiError(httpStatus.UNAUTHORIZED, 'Invalid credentials. Please check your email/mobile and password.');
  }
...
/**
 * Sign in with a Google ID token issued to the client. The token is verified
 * server-side, then we find-or-create a `user` account from its claims.
 */
